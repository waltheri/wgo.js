...
"string"==typeof a?a.replace(/</g,"&lt;").replace(/>/g,"&gt;"):a};var g=function(a,b){b=b||{};for(var c in b)this[c]=b[c];for(c in h.Board.default)this[c]===s&&(this[c]=h.Board.default[c]);for(c in g.themes.default)this.theme[c]===s&&(this.theme[c]=g.themes.default[c]);this.tx=this.section.left;this.ty=this.section.top;this.bx=this.size-1-this.section.right;this.by=this.size-1-this.section.bottom;this.init();a.appendChild(this.element);this.pixelRatio=v.devicePixelRatio||1;this.width&&this.height?
...
a)},D=function(a,b,c){return a.obj_arr[b][c][0]&&a.obj_arr[b][c][0].c==h.W||a.obj_arr[b][c][0].c==h.B},w,y=function(a){for(var b=a.angle,c=a.angle,e=0;e<a.lines.length;e++){var b=b+a.lines[e],c=c-a.lines[e],d=a.ctx,f=a.x,h=a.y,g=a.radius,k=b,n=c,m=a.factor,l=a.thickness;d.strokeStyle="rgba(64,64,64,0.2)";d.lineWidth=g/30*l;d.beginPath();var g=g-Math.max(1,d.lineWidth),l=f+g*Math.cos(k*Math.PI),k=h+g*Math.sin(k*Math.PI),f=f+g*Math.cos(n*Math.PI),h=h+g*Math.sin(n*Math.PI),q=n=void 0,q=n=void 0;f>l?
...
2*d/5,e-2*d/5,1,c-d/5,e-d/5,3*d/5),f.addColorStop(0,"#555"),f.addColorStop(1,"#000"));this.beginPath();this.fillStyle=f;this.arc(c-b.ls,e-b.ls,Math.max(0,d-.5),0,2*Math.PI,!0);this.fill()}},shadow:m},SHELL:{stone:{draw:function(a,b){var c,e,d=b.stoneRadius;w=w||Math.ceil(9999999*Math.random());c=b.getX(a.x);e=b.getY(a.y);var f;f=a.c==h.W?"#aaa":"#000";this.beginPath();this.fillStyle=f;this.arc(c-b.ls,e-b.ls,Math.max(0,d-.5),0,2*Math.PI,!0);this.fill();if(a.c==h.W){f=w%(3+a.x*b.size+a.y)%3;var g=b.size*
b.size+a.x*b.size+a.y,g=2/g*(w%g);0==f?y({ctx:this,x:c,y:e,radius:d,angle:g,lines:[.1,.12,.11,.1,.09,.09,.09,.09],factor:.25,thickness:1.75}):1==f?y({ctx:this,x:c,y:e,radius:d,angle:g,lines:[.1,.09,.08,.07,.06,.06,.06,.06,.06,.06,.06],factor:.2,thickness:1.5}):y({ctx:this,x:c,y:e,radius:d,angle:g,lines:[.12,.14,.13,.12,.12,.12],factor:.3,thickness:2});f=this.createRadialGradient(c-2*d/5,e-2*d/5,d/3,c-d/5,e-d/5,5*d/5);f.addColorStop(0,"rgba(255,255,255,0.9)");f.addColorStop(1,"rgba(255,255,255,0)")}else f=
...
this):E.call(this[d].getContext(this.obj_arr[a][b][e]),this.obj_arr[a][b][e],this)}},x=function(a,b){var c,e;for(e in this.obj_arr[a][b]){c=this.obj_arr[a][b][e].type?"string"==typeof this.obj_arr[a][b][e].type?g.drawHandlers[this.obj_arr[a][b][e].type]:this.obj_arr[a][b][e].type:this.stoneHandler;for(var d in c)c[d].draw.call(this[d].getContext(this.obj_arr[a][b][e]),this.obj_arr[a][b][e],this)}},F=function(a){var b;b=a.layerX*this.pixelRatio;b-=this.left;b/=this.fieldWidth;b=Math.round(b);a=a.layerY*
this.pixelRatio;a-=this.top;a/=this.fieldHeight;a=Math.round(a);return{x:b>=this.size?-1:b,y:a>=this.size?-1:a}},C=function(){this.element.style.width=this.width/this.pixelRatio+"px";this.element.style.height=this.height/this.pixelRatio+"px";this.stoneRadius=k("stoneSize",this);this.ls=k("linesShift",this);for(var a in this.layers)this.layers[a].setDimensions(this.width,this.height)};g.prototype={constructor:g,init:function(){this.obj_arr=[];for(var a=0;a<this.size;a++){this.obj_arr[a]=[];for(var b=
...
100);this.addLayer(this.shadow,200);this.addLayer(this.stone,300)},setWidth:function(a){this.width=a;this.width*=this.pixelRatio;this.fieldHeight=this.fieldWidth=4*this.width/(4*(this.bx+1-this.tx)+2);this.left=z.call(this);this.height=(this.by-this.ty+1.5)*this.fieldHeight;this.top=A.call(this);C.call(this);this.redraw()},setHeight:function(a){this.height=a;this.height*=this.pixelRatio;this.fieldWidth=this.fieldHeight=4*this.height/(4*(this.by+1-this.ty)+2);this.top=A.call(this);this.width=(this.bx-
this.tx+1.5)*this.fieldWidth;this.left=z.call(this);C.call(this);this.redraw()},setDimensions:function(a,b){this.width=a||parseInt(this.element.style.width,10);this.width*=this.pixelRatio;this.height=b||parseInt(this.element.style.height,10);this.height*=this.pixelRatio;this.fieldWidth=4*this.width/(4*(this.bx+1-this.tx)+2);this.fieldHeight=4*this.height/(4*(this.by+1-this.ty)+2);this.left=z.call(this);this.top=A.call(this);C.call(this);this.redraw()},getSection:function(){return this.section},setSection:function(a,
...
0;a<this.layers.length;a++)this.layers[a].clear(this),this.layers[a].draw(this);for(a=0;a<this.size;a++)for(var b=0;b<this.size;b++)x.call(this,a,b);for(var c in this.obj_list){var a=this.obj_list[c].handler,e;for(e in a)a[e].draw.call(this[e].getContext(this.obj_list[c].args),this.obj_list[c].args,this)}},getX:function(a){return this.left+a*this.fieldWidth},getY:function(a){return this.top+a*this.fieldHeight},addLayer:function(a,b){a.appendTo(this.element,b);a.setDimensions(this.width,this.height);
this.layers.push(a)},removeLayer:function(a){var b=this.layers.indexOf(a);0<=b&&(this.layers.splice(b,1),a.removeFrom(this.element))},update:function(a){if(a.remove&&"all"==a.remove)this.removeAllObjects();else if(a.remove)for(var b in a.remove)this.removeObject(a.remove[b]);if(a.add)for(b in a.add)this.addObject(a.add[b])},addObject:function(a){if(a.constructor==Array)for(var b in a)this.addObject(a[b]);else{B.call(this,a.x,a.y);for(b in this.obj_arr[a.x][a.y])if(this.obj_arr[a.x][a.y][b].type==
a.type){this.obj_arr[a.x][a.y][b]=a;x.call(this,a.x,a.y);return}a.type?this.obj_arr[a.x][a.y].push(a):this.obj_arr[a.x][a.y].unshift(a);x.call(this,a.x,a.y)}},removeObject:function(a){if(a.constructor==Array)for(var b in a)this.removeObject(a[b]);else{var c;for(b=0;b<this.obj_arr[a.x][a.y].length;b++)if(this.obj_arr[a.x][a.y][b].type==a.type){c=b;break}c!==s&&(B.call(this,a.x,a.y),this.obj_arr[a.x][a.y].splice(c,1),x.call(this,a.x,a.y))}},removeObjectsAt:function(a,b){this.obj_arr[a][b].length&&(B.call(this,
...
(a=this.stack.pop(),this.turn=0==this.stack.length?h.B:this.position.color?-this.position.color:-this.turn);return a},firstPosition:function(){this.stack=[];this.stack[0]=new r(this.size);this.stack[0].capCount={black:0,white:0};this.turn=h.B;return this},getCaptureCount:function(a){return a==h.B?this.position.capCount.black:this.position.capCount.white},validatePosition:function(){for(var a,b,c=0,e=0,d=[],f=this.position.clone(),g=0;g<this.size;g++)for(var k=0;k<this.size;k++)if(a=this.position.get(g,
k))b=d.length,d=d.concat(l(f,g-1,k,-a),l(f,g+1,k,-a),l(f,g,k-1,-a),l(f,g,k+1,-a)),a==h.B?e+=d-b:c+=d-b;this.position.capCount.black+=e;this.position.capCount.white+=c;this.position.schema=f.schema;return d}};h.Game=m;v.WGo=h})(window);
