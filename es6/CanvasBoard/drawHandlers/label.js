...
export default {
	stone: {
		draw: function (canvasCtx,  args, board) {
			...
			if (args.text.length == 1) canvasCtx.font = Math.round(stoneRadius * 1.5) + "px " + font;
			else if (args.text.length == 2) canvasCtx.font = Math.round(stoneRadius * 1.2) + "px " + font;
			else canvasCtx.font = Math.round(stoneRadius) + "px " + font;
